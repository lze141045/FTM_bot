...
WICHTIG: Dieser Code verschickt alle Nachrichten als reinen Text (kein
Markdown/HTML-Parsing). Das ist absichtlich so, damit Sonderzeichen wie
_ * [ ] in deinen eigenen Texten (z.B. @dein_username, IBANs, Links)
niemals zu einem "400 Bad Request"-Fehler führen können.
"""
...
#  (Sonderzeichen wie _ oder * sind hier jetzt unproblematisch)
# ============================================================
...
PAYMENT_INFO = "PayPal: deine@email.com oder IBAN: DE00 0000 0000 0000 00"
...
OPTION_A_TEXT = (
    ...
    f"1) Registriere dich über diesen Link:\n{AFFILIATE_LINK}\n\n"
    f"2) Zahle die Mindesteinzahlung ({MIN_EINZAHLUNG}) beim Broker ein\n\n"
    f"3) Schick einen Screenshot deiner Einzahlung an {SUPPORT_CONTACT}\n\n"
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if args and args[0] == "broker":
        ...
        await update.message.reply_text(
            OPTION_A_TEXT, reply_markup=group_choice_keyboard()
        )
    elif args and args[0] == "abo":
        context.user_data["payment_choice"] = "Monatliches Abo (Option B)"
        await update.message.reply_text(
            OPTION_B_TEXT, reply_markup=group_choice_keyboard()
        )
    else:
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if query.data == "group_safe":
        ...
        await query.message.reply_text(SAFE_REPLY)
    elif query.data == "group_risk":
        group_choice = "Riskante Signale"
        await query.message.reply_text(RISK_REPLY)
    else:
        return
